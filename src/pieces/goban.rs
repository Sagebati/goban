...
impl Goban {
    ...
    pub fn number_of_stones(&self) -> (u32, u32) {
        self.go_strings
            ...
            .fold((0, 0), |(x1, x2), go_str_ptr| match go_str_ptr.color {
                Color::Black => (x1 + go_str_ptr.stones().len() as u32, x2),
                Color::White => (x1, x2 + go_str_ptr.stones().len() as u32),
                _ => unreachable!(),
            })
    }
        ...
        for p in self.neighbor_points(point) {
            match &self.go_strings[self.coord_util.to(p)] {
                Some(adj_go_str_ptr) => match adj_go_str_ptr.color {
                    ...
                    Color::None => {
                        unreachable!("A string cannot be of color none")
                    }
                    _ => {
